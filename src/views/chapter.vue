...
export default {
  ...
  methods: {
    ...mapActions(["initChapter", "updateChapter"]),
    ...
    visibleChapterIndex() {
      ...
      for (let i = list.length - 1; i >= 0; i--) {
        const item = list[i];
        if (
          item.offsetTop + item.offsetHeight + innerHeight >= pageYOffset &&
          item.offsetTop <= pageYOffset + innerHeight
        ) {
          return i;
        }
      }
...
<style lang="less">
@import "../assets/css/chapter.less";
</style>
